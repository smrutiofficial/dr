...
placeholder_icon = "/usr/share/icons/hicolor/256x256/apps/broken-image.png"
# placeholder_icon = ""
...
def get_gui_apps_with_updates_and_icons():
    try:
        ...
        apt_update_cmd = "pkexec apt list --upgradable"
...
def update_apt_app(app_name):
    try:
        subprocess.run(['pkexec', 'apt-get', 'upgrade', app_name, '-y'], check=True)
    ...
    if package_type == "APT":
        try:
            ...
            result = subprocess.run(
                ['pkexec', 'apt', 'remove', '-y', app_id['name']],
                stdout=subprocess.PIPE,
...
app = ctk.CTk(fg_color="#151c26",className='Dr.Linux')
